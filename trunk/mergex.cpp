...
int main(int argc, char* argv[]) {
	if (argc < 2) {
		cout << argv[0] << " v2.1:" << endl << "\tMerge XML files to std output or an output file."<<endl<<"\tALL XML should have the same xml prolog and root node." << endl;
		...
		cout << endl << "\t-a[ParentRootAttribute]\tAdd an attribute to the parent root.\n\t\t\t\tParentRootAttribute should be a valid xml attribute\n\t\t\t\tdefined with single quote.\n\t\t\t\tIt will only work with the -p parameter." << endl;
		...
		cout << "\tmergex -ooutput.xml -pROOT -avalue='test' *.xml"<<endl;
		cout << "\tmergex -ooutput.xml -p -avalue='test' -aAnotherAttribute=\'anothertest\' *.xml"<<endl;
		return 0;
	}

	bool createParent(false);
	string outputFilePath,firstXMLFilePath,parentRootName,parentRootAttribute;
	for (int i=1; i<argc; i++) {
		string param(argv[i]);
		createParent = createParent || param.compare(0, 2, "-p") == 0;
		if (param.compare(0, 2, "-p") == 0) {
			parentRootName = param.substr(2);
		} else if (param.compare(0, 2, "-a") == 0) {
			parentRootAttribute += " " + param.substr(2);
		...
		if (found) {
			...
			if (createParent) {
				...
				if ((pos = header.find(startRootNodeKey)) != string::npos) {
					header = header.substr(0,pos)+"<"+parentRootName+parentRootAttribute+">";
				}
			}
